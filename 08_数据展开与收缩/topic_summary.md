...
* 行转列常用条件聚合。
* 多行收缩为一行可用 `collect_list()` / `collect_set()` 和 `concat_ws()`。
...
|---|---|---|---|---|---|
| 001 | `001_字符串区间展开.md` | 字符串区间展开 | split、posexplode、区间补全、collect_list 拼接 | 否 | 是 |
| 002 | `002_数据收缩合并.md` | 数据收缩合并 | collect_list、concat_ws、全局收缩、连续段收缩 | 否 | 是 |
...
* 多值拼接前没有去重或排序。
* 列转行后字段类型不一致。
* 连续段收缩时直接按取值分组，导致中间被其他值打断后的同值记录被错误合并。
